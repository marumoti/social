...
<div class="main_column column" id="main_column">
	...
	if($query == "")
		...
		if(mysqli_num_rows($usersReturnedQuery) == 0)
			...
			echo mysqli_num_rows($usersReturnedQuery) . " 人の結果: <br> <br>";


		echo "<p id='grey'>見つからなかった場合次の方法で検索してみてください:</p>";
		...
		while($row = mysqli_fetch_array($usersReturnedQuery)) {
			...
			if($user['username'] != $row['username']) {
				...
				if($user_obj->isFriend($row['username']))
					...
					$button = "<input type='submit' name='" . $row['username'] . "' class='warning' value='リクエストに応答'>";
				else if($user_obj->didSendRequest($row['username']))
					$button = "<input type='submit' class='default' value='Request Sent'>";
				else 
					$button = "<input type='submit' name='" . $row['username'] . "' class='success' value='友達に追加'>";

				$mutual_friends = $user_obj->getMutualFriends($row['username']) . " 人の共通の友達";
